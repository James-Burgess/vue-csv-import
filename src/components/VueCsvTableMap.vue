<template>
    ...
    >
        <template v-if="VueCsvImportData.firstSampleRow">
            <table id="csv-table" v-bind="tableAttributes">
              <thead>
                <tr>
                  <td v-for="(field, key) in headers" :key="field">
                    ...
                    <option value>{{ labels.excludeField }}</option>
                      ...
                      >
                            {{ option.label }}{{ labels.requiredField }}
                      </option>
                      <option
              ...
              <tbody>
                <tr v-for="row in VueCsvImportData.fileHasHeaders ? VueCsvImportData.csvSample : VueCsvImportData.csvSample.slice(1)">
        ...
        setup(props) {
            ...
            const headers = computed(() => {
                if (VueCsvImportData.fileHasHeaders) {
                    return [...Array(VueCsvImportData.firstSampleRow.length).keys()].map(i => `Column ${i}`)
                } else {
